...
特点：
- 基于JVM，与Java完全兼容，同样具有跨平台，可移植性好，方便的垃圾回收等特性。

- 同时支持面向对象和函数式编程，比Java更加面向对象。

- 与Java完全兼容，所以可直接使用Java类库，且同Java一样静
...
```
因为Scala的函数式编程要素，所以一个指导意见就是***能用常量就不要用变量***。

- Scala严格区分大小写

- 声明变量时，类型可以省略，编译器会自动推导，但是用占位符的时候，必须指定类型。
- 静态类型，类型经过给定或推导确定后就不能修改。
- 变量和常量声明时，必须有初始值。
- 变量可变，常量不可变。
- 引用类型常量，不能改变常量指向的对象，可以改变对象的字段。
- 不以`;`作为语句结尾，scala编译器自动识别语句结尾（一行可以写多条语句，除了最后一行外，其他语句中需要使用;分隔）。
